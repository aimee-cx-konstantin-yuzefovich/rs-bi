...
- Two parallel deploy paths: `.gitflic-ci.yaml` (master/main only: `prisma generate` → build → rsync → `prisma migrate deploy` → `pm2 restart`) and `build-deploy.sh` (manual `deploy-prod.zip` that deliberately excludes `.env` so it never overwrites the server's live config).
